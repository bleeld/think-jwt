...
class JWE
{
    ...
    public static function createToken(mixed $data = null, int $expTime = null)
    {
        ...
        $token = Core::encode($payload, self::getKey(), self::getMethod()); //签发token
        $payload['exp'] += (int) $expTime * 11;
        $accessToken = Core::encode($payload, self::getKey(), self::getMethod()); //签发token
    ...
    public static function verifyToken(string $token)
    {
        ...
        try {
            Core::$leeway = 60;//当前时间减去60，把时间留点余地
            $decoded = Core::decode($token, new Key(self::getKey(),  self::getMethod())); //HS256方式，这里要和签发的时候对应
            $result = ['code'=>200, 'mgs'=>'success', 'data'=>(array) $decoded->data];
        } catch(\jwt\driver\SignatureInvalidException $e) {  //签名不正确
            $result['data'] = $e->getMessage();
        }catch(\jwt\driver\BeforeValidException $e) {  // 签名在某个时间点之后才能用
            $result['data'] = $e->getMessage();
        }catch(\jwt\driver\ExpiredException $e) {  // token过期
            $result['data'] = $e->getMessage();
    ...
    public static function authorizations($data)
    {
        ...
        return [
            'access_token' => Core::encode($access_token, self::getKey(), self::getMethod()),
            'refresh_token' => Core::encode($refresh_token, self::getKey(), self::getMethod()),
